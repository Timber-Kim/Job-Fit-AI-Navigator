...
with st.sidebar:
    ...
    # 직무 선택 (DB 기반 자동 정렬)
    job_options = ["직접 입력"] + clean_job_titles()
...
if selected_job != "직접 입력" and selected_situation != "직접 입력":
    outs = ", ".join(output_format) if output_format else ""
    q_text = f"직무: {selected_job}, 상황: {selected_situation}, 필요결과물: {outs}. 적합한 AI 도구 추천해줘."
    st.button(f"🔍 '{selected_job}' - '{selected_situation}' 추천받기", type="primary", 
              on_click=ask_ai, args=(q_text,), use_container_width=True)
